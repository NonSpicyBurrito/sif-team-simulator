...
import { useDebounce, useInterval, useLocalStorage } from '@vueuse/core'
    ...
    return conditions.reduce(
        (ids, condition) => {
            ...
            if (effects[condition]) {
                return ids.filter(
                    (id) =>
                        cards.get(id)?.skill.effect.type === effects[condition]
                )
            }
            ...
            const allowedCharacters = [...characters.entries()]
                .filter(([, name]) => name.toLowerCase().includes(condition))
